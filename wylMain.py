...
def buildModel():
    ...
    #model = applications.VGG19( include_top=False, weights='imagenet', input_shape = input_shape, )
    #model = applications.ResNet50( include_top=False, weights='imagenet', input_shape = input_shape, )
    ...
    model = applications.MobileNet( include_top=False, weights='imagenet', input_shape = input_shape, )
    #########################################################################################
    # initialise top model
    # for layer in model.layers: #冻结VGG16的所有层
    #     layer.trainable = False

    # top_tensor = Conv2D(512, (3, 3), activation='relu', padding='same', name='block6_conv1')(model.output)
    # top_tensor = Conv2D(512, (3, 3), activation='relu', padding='same', name='block6_conv2')(top_tensor)
    # top_tensor = Conv2D(512, (3, 3), activation='relu', padding='same', name='block6_conv3')(top_tensor)
    # top_tensor = MaxPooling2D((2, 2), strides=(2, 2), name='block6_pool')(top_tensor)

    top_tensor = Flatten(name='flatten')(model.output)
    top_tensor = Dense(2048, activation='relu', name='fc1')(top_tensor)
    #top_tensor = Dropout(0.35)(top_tensor)
    top_tensor = Dense(512, activation='relu', name='fc2')(top_tensor)
    #top_tensor = Dropout(0.35)(top_tensor)
    top_tensor = Dense(100, activation='softmax', name='predictions')(top_tensor)

    model = Model( model.input, top_tensor, name='vgg16')
    #######################################################################
    model.compile(
