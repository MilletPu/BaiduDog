...
model_weights_file = "./models/Xception_weight_"+CUDA_VISIBLE_DEVICES+".h5"
sub_file = "./sub/Xception"+CUDA_VISIBLE_DEVICES+"_submission.txt"
cnn_model_file = "./models/Xception"+CUDA_VISIBLE_DEVICES+"Recent.model"
...
def buildModel( init = False ):
    ...
    top_tensor = Dense(512, activation='sigmoid', name='fc1')(top_tensor)
    ...
    if init==True: # 冻结的所有卷积层,预处理全连接层
        ...
        model.compile (
            loss='categorical_crossentropy',
            optimizer=optimizers.Adadelta(lr=1e-4, rho=0.95, epsilon=1e-06),
            #optimizer=optimizers.SGD(lr=1e-4, momentum=0.9),
            #optimizer='adam', 
            metrics=['accuracy'],
        )   
