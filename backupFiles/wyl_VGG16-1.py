...
def VGGCAM(nb_classes, num_input_channels=1024):
    ...
    model.add(Conv2D(num_input_channels, (3, 3), activation='relu', border_mode="same"))
...
def train_VGGCAM(VGG_weight_path, nb_classes, num_input_channels=1024):
    ...
    with h5py.File(VGG_weight_path) as hw:
        for k in range(hw.attrs['nb_layers']):
            ...
            if model.layers[k].name == "conv2d_13":
                break
        print('Model loaded.')
...
def plot_classmap(VGGCAM_weight_path, img_path, label, nb_classes, num_input_channels=1024, ratio=16):
    """
